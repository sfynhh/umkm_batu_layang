...
class AuthController extends Controller
{
    ...
    public function attemptRegister()
    {
        ...
        $id_user=$this->UM->max_iduser();
        $id_pemilik =$this->UM->max_idpemilik();
        $datapemimilik=array(
        ...
        $savedatapemilik=$this->UM->createdatapemilik($datapemimilik);
        $savedatamitra=$this->UM->createdatamitra($datamitra);
        // Ensure default group gets assigned if set
        if (! empty($this->config->defaultUserGroup)) {
            $users = $users->withGroup($this->config->defaultUserGroup);
        }

        if (! $users->save($user) || ! $savedatapemilik || ! $savedatamitra ) {
